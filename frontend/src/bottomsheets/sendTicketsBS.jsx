...
class SendTicketBS extends Component {
	...
	addContactsToTicketList = () => {
		const ticketList = [...this.state.chooseTicketHolder];
		const selectedContacts = [...this.state.selectedContacts];
		const ticketsWithContacts = [];

		ticketList.forEach((element, index) => {
			ticketsWithContacts.push({
				person: selectedContacts[index],
				ticket: element,
			});
		});

		this.sendOnSMS(ticketsWithContacts);
	};

	sendOnSMS = (ticketsWithContacts) => {
		console.log('tickets with contact: ', ticketsWithContacts);

		ticketsWithContacts.forEach((element) => {
			const text = {
				recipient: element.person.phoneNumber,
				textmessage:
					'Billett kjøpt til ' +
					...
					element.ticket.type +
					', Referanse kode: ' +
					element.ticket.referenceCode,
			};

			console.log('text: ', text.textmessage, 'recpient: ', text.recipient);
	...
	render() {
		...
		return (
			<div className="w-full z-10 absolute bottom-0 h-auto bg-white rounded-t-md modal">
				<div className="">
					<HeaderSendTickets
						end={this.props.endSendingTickets}
					></HeaderSendTickets>
					{this.reviewTicket()}
